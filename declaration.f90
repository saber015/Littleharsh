module declaration
  ...
  integer dsty,dstx,dstz,qp
  integer nsolid, nfluid
  integer nelements_y
  ...
  ! Variables for fibers with circular section 12.11.2019
  real(8) :: jcentre, kcentre
  real(8) :: radius, dcy, dcz      ! Radius, y and z components of the distance
  real(8) :: dzp                   ! Deltaz, used to calculate dc
  real(8) :: acoef                 ! Coefficient for index lists, defined as ratio of area 18.11.2019
  real(8) ds, diff                 !20.11.2019
  ...
  ! Low pass filter function, applied to nonlinear and IB terms 19.4.2020
  real(8) K_lpf
  real(8) xi1, xi2 ! decay constants

  !no need for these to be pointers, they should be allocatable arrays (they are more efficient)
  integer, pointer :: nlist_s(:) !, n_nylim(:)
  integer, pointer :: nlist_f(:) ! 5.2.2020
  integer, pointer :: s_list_u(:,:), s_list_v(:,:)
  integer, pointer :: f_list_u(:,:), f_list_v(:,:)
  real(8), pointer :: w_list_u(:,:), w_list_v(:,:)
  integer, pointer :: nyuIB1(:),nyuIB2(:),nyvIB1(:),nyvIB2(:) ! 6.2.2020
  real(8), pointer :: A_ib(:,:,:)
  integer          :: nyu11,nyu21,nyu12,nyu22,nyv11,nyv21,nyv12,nyv22
  ! The following arrays are changed from 1D to 2D to indentify curved boundary
  ! 5.12.2019
  integer, allocatable :: nyu11_list(:,:), nyu21_list(:,:)
  integer, allocatable :: nyu12_list(:,:), nyu22_list(:,:)
  integer, allocatable :: nyv11_list(:,:), nyv21_list(:,:)
  integer, allocatable :: nyv12_list(:,:), nyv22_list(:,:)

  real(8), allocatable :: nyu11_ratio(:,:), nyu21_ratio(:,:)
  real(8), allocatable :: nyu12_ratio(:,:), nyu22_ratio(:,:)
  real(8), allocatable :: nyv11_ratio(:,:), nyv21_ratio(:,:)
  real(8), allocatable :: nyv12_ratio(:,:), nyv22_ratio(:,:)
  ...
  type(cfield), allocatable:: u1_itp(:),u2_itp(:),u3_itp(:)  ! 9.3.2020
  type(cfield), allocatable:: Nu1_dy(:),Nu2_dy(:),Nu3_dy(:)  ! 9.3.2020
  type(cfield), allocatable:: uv_f(:), vw_f(:), vv_c(:)      ! 9.3.2020
