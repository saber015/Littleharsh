...
nextqt = floor(t*10d0)/10d0+0.1d0
  ...
  !do while (t<maxt .AND. iter < 1)

      ! Eliminate the pressure oscillation by setting p^n = 0 every number of iterations
      ! 12.4.2020
...
!call MPI_BARRIER(MPI_COMM_WORLD,ierr)   ! 6.3.2020
!if (myid .eq. 0) then
!write(*,*) 'myid =', myid
!do iband = sband,eband
!    do column = 1,columns_num(iband,myid)
!      do j=jlim(1,grid,iband)+1,jlim(2,grid,iband)-1
!         write(*,*) 'rhsu =', du1(iband)%f(-79,column)  
!      enddo
!    enddo
!  enddo
!end if 
...
!do iband = sband,eband
!    do column = 1,columns_num(iband,myid)
!      do j=nyuIB1(myid), nyuIB2(myid)!jlim(1,grid,iband)+1,jlim(2,grid,iband)-1
!         write(*,*) '   j =', j
!         write(*,*) ' Nu1 =', Nu1(sband)%f(j,3)
!         write(*,*) '  u1 =', u1(sband)%f(j,3)
!         write(*,*) 'rhsu =', du1(sband)%f(j,3)  
!      enddo
!!    enddo
!!  enddo
!end if 
...
!call MPI_BARRIER(MPI_COMM_WORLD,ierr)
!if (myid .eq. 0) then
!   write(*,*) 'This is Runge-Kuta substep', kRK
!   write(*,*) 'Continue ? 0 - No, 1 - Yes'
!   read(*,*) stepblowup
!   if (stepblowup .eq. 1) then
!      go to 111
!   else 
!      stop
!   end if
...
! enddo
  ...
  !call low_pass_filter(psi,ugrid,myid)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  include 'mpif.h'             ! MPI variables
  integer status(MPI_STATUS_SIZE),ierr         ! 6.3.2020
...
!    do column = 1,columns_num(iband,myid)
!      do j=jlim(1,grid,iband)+1,jlim(2,grid,iband)-1
!         write(*,*) 'rhsu =', du(iband)%f(-79,column)  
!      enddo
!    enddo
!  enddo
!end if 
...
!if (myid .eq. 0) then
!!write(*,*) 'myid =', myid
!open(unit=11,file='du_in_solveU.dat')
!rewind 11
! iband = sband
!    do column = 1,columns_num(iband,myid)
!      do j=jlim(1,ugrid,iband)+1,jlim(2,ugrid,iband)-1
!         write(11,*) 'column, j =', column, j
!         write(11,*) '       du =', du(iband)%f(j,column)  
!      enddo
!    enddo
!  enddo
!close(unit=11)
!end if 
!call MPI_BARRIER(MPI_COMM_WORLD,ierr)
  call immersed_boundaries_U(du,u,ugrid,myid)

  call LUsolU(du,u,ugrid,myid) ! 25.4.2020
...
!         u(iband)%f(j,column) = u(iband)%f(j,column)+du(iband)%f(j,column)
      enddo
    enddo
  end do

!  call low_pass_filter(u,ugrid,myid)
...
!         du(iband)%f(j,column) = dt*(du(iband)%f(j,column)) !For solving for du
  ...
  call immersed_boundaries_V(du,u,vgrid,myid)

  call LUsolV(du,u,vgrid,myid) ! 25.4.2020
...
!  call low_pass_filter(u,vgrid,myid)
...
!        w_out(iband)%f(j,column) = u3(iband)%f(j,column)
  ...
  !call low_pass_filter(u1,ugrid,myid)
  !call low_pass_filter(u2,vgrid,myid)
  !call low_pass_filter(u3,ugrid,myid)
end subroutine

subroutine low_pass_filter(x,grid,myid)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!! Exponential filter of order xi1 !!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if (grid .eq. ugrid) then
     allocate (xPL(igal,kgal,nyuIB1(myid):nyuIB2(myid)))
     j1 = nyuIB1(myid)
     j2 = nyuIB2(myid)
     call modes_to_planes_dU(xPL,x,myid,status,ierr)
  else 
     allocate (xPL(igal,kgal,nyvIB1(myid):nyvIB2(myid)))
     j1 = nyvIB1(myid)
     j2 = nyvIB2(myid)
     call modes_to_planes_dV(xPL,x,myid,status,ierr)
  end if

  do j = j1, j2
     do i = 0, Ngal(1,1)/2
        do k = 1, Ngal(2,1)/2
           K_lpf = exp(xi2*(2*pi*i*1d0/(Ngal(1,1)/2))**xi1) &
               & * exp(xi2*(2*pi*(k-1)*1d0/Ngal(2,1))**xi1)

           xPL(2*i+1,k,j) = K_lpf * xPL(2*i+1,k,j)
           xPL(2*i+2,k,j) = K_lpf * xPL(2*i+2,k,j)
        end do
        do k = Ngal(2,1)/2+1,Ngal(2,1)
           K_lpf = exp(xi2*(2*pi*i*1d0/(Ngal(1,1)/2))**xi1) &
               & * exp(xi2*(2*pi*(-Ngal(2,1)+k-1)*1d0/Ngal(2,1))**xi1)
  ...
  if (grid .eq. ugrid) then
     call planes_to_modes_dU(x,xPL,myid,status,ierr)
  else 
     call planes_to_modes_dV(x,xPL,myid,status,ierr)
  end if
