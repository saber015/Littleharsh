...
program littleharsh
   ...
   integer iband
   type(cfield), allocatable:: u1(:),u2(:),u3(:),p(:),psi(:) !P(n+1) = P(n)+psi(n+1); n: time step
   type(cfield), allocatable:: div(:)
   type(cfield), allocatable:: Nu1(:),Nu2(:),Nu3(:) !dealialized advective terms
   type(cfield), allocatable:: du1(:),du2(:),du3(:)
   type(cfield), allocatable:: Lu1(:),Lu2(:),Lu3(:)
  
   real(8):: z,sigmaz !!!!!not used
   integer column,i,k,j

   real(8):: flagslstat !!!!!not used
   ...
   do iband = sband,eband !sband = 1; eband = nband = 3
