...
!             ygrid (obsolete) - Called from start
...
subroutine start(myid,status,ierr)
   ...
   iproc = 0
   do while (pnodes<np) !np: Number of processes in the group of comm
      iproc  = iproc+1
      pnodes = 2**iproc
   end do
   ...
   if (myid==0) then
      ...
      read(40,10) posth         ! riblet height to width ratio; canopy height
      ...
      elseif (geometry_type == 3) then
         post_spacing = Lz/ntilez
         !posth        = posth
         h_ny(1)      = 1.5d0*posth !!!!! height of band 1 above canopy
      else
         post_spacing = Lz/ntilez
   ...
   nstat = min(20,nwrite)! call spectra nstat iteration

   ! Initialise the FFT
   allocate(buffR_x(nband),buffC_z(nband))   !real,complex buffer, for fft in x and z
...
30 FORMAT(7X,A90)
   ...
   call proc_lims_planes (myid)!planes (load) per proc
...
subroutine getbounds(myid,status,ierr)
   ...
   if (geometry_type/=3) then
      ...
      if (myid==0) then
         ...
         allocate(list_ib1(3,nlist_ib1,2),list_ib2(3,nlist_ib2,2))
         ...
         do iproc = 1,np/2-1
            ...
            allocate(list_ib(3,nlist_ib(ugrid),2))
            ...
            call MPI_SEND(list_ib,3*nlist_ib(ugrid)*2,MPI_INTEGER,iproc,21000+iproc,MPI_COMM_WORLD,ierr)
      ...
      ! SLAVES
         ...
         !      allocate(A_ib   (1,nlist_ib(ugrid),2))
         ! ATTENTION nlist_ib ugrid and vgrid are equal, otherwise it wont work
         call MPI_RECV  (list_ib,3*nlist_ib(ugrid)*2,MPI_INTEGER,0,21000+myid,MPI_COMM_WORLD,status,ierr)
...
subroutine proc_lims_planes(myid)
   ...
   real(8) :: loadT, loadP, maxload, proc_ratio
   ...
   proc_ratio     = load_band(botband)*1d0/(load_band(midband)*1d0)
   procs(botband)  = floor(proc_ratio*np/(proc_ratio*2+1))
