...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  integer iband,iproc,inputInt(16),k
  integer i
...
!  and send this data to the other procs.
  if (myid==0) then
    ...
    !nb of internal points, without accounting for the boundaries
    ...
    maxA = -maxA
    read(40,20) physlim_bot   ! NOTUSED Defines the last physical space plane (bottom band) for tridiag solve
    read(40,20) physlim_top   ! NOTUSED Defines the last physical space plane (top band) for tridiag solve
    read(40,10) xi1           ! Decay constant in low pass filter function ! 19.4.2020
    read(40,10) xi2           ! Decay constant in low pass filter function ! 19.4.2020
    read(40,10)               ! - Blank
    read(40,10)               ! - Blank

    ! Surface geometry
    read(40,20) geometry_type ! 0 smooth, 1 hydrophobic, 2 roughness, 3 canopies, 4 substrates
    read(40,10)               ! - Blank
    read(40,10)               ! - Blank
    read(40,10)               ! - Blank
    read(40,10)               ! - Blank
    ...
    read(40,10) posth         ! height of porous or canopy medium in outer units
    read(40,20) npeakx        ! NOTUSED number of points in x representing the blade tip (roughness)
    read(40,20) npeakz        ! NOTUSED number of points in z representing the blade tip (roughness)
    read(40,10) Lfracx        ! NOTUSED number of points representing the blade tip
    read(40,10) Lfracz        ! NOTUSED number of points representing the blade tip
    read(40,20) dsty          ! number of points representing solid element in y for canopies and substrates 
    read(40,20) dstx          ! number of points representing solid element in x for canopies and substrates
    read(40,20) dstz          ! number of points representing solid element in z for canopies and substrates
    read(40,20) nsolid        ! number of points representing solid element in y (vgrid) for permeable substrates in the constant-dy region
    read(40,20) nfluid        ! number of points representing fluid element in y (vgrid) for permeable substrates in the constant-dy region
    ...
    if (geometry_type == 3) then
      if (dstx <= 0 .OR. dstz <= 0 .OR. dsty <= 0 .OR. nsolid <= 0 .OR. nfluid <= 0) then
        write(*,*) 'The number of points within the substrate must be positive'
        stop
      end if 
    end if 
    
    if (geometry_type == 4) then
...
! The master thread send to the others all the information they'll need
    ...
    do iproc=1,np-1
      ...
      call MPI_SEND(inputInt,                     16,MPI_INTEGER,iproc,  1000+iproc,MPI_COMM_WORLD,ierr)
...
! Every proc knows the physical parameters and the whole geometry describing the problem (including N and Ngal)
    ...
    call MPI_RECV(inputInt,16,MPI_INTEGER,0,1000+myid,MPI_COMM_WORLD,status,ierr)
    Re        = inputR( 1)
    alp       = inputR( 2)
    bet       = inputR( 3)
    Lx        = inputR( 4)
...
30 FORMAT(7X,A90)
  ...
  !  If the planes that a proc has to solve are in the middle of the channel, the don't have to deal with the
  !  immersed boundaries, and then the list is empty.
  ! list_ib      stores the 'solid' points at the immersed boundary
  ! list_g       stores the 'ghost' points where velocity is imposed. 3.2.2020
...
if (myid .eq. 8) then
open(unit=60,file='y_limits_phys_8.dat')
!rewind 60
write(60,*) 'myid, nyuIB1, nyuIB2 =', myid,             nyuIB1(myid),             nyuIB2(myid)
write(60,*) 'myid,  jlim1,  jlim2 =', myid,      jlim(1,ugrid,sband),      jlim(2,ugrid,sband)
write(60,*) 'myid,  jgal1,  jgal2 =', myid,            jgal(ugrid,1),            jgal(ugrid,2)
write(60,*) 'myid,limPL1i,limPL2i =', myid, limPL_incw(ugrid,1,myid), limPL_incw(ugrid,2,myid)
write(60,*) 'myid,limPL1e,limPL2e =', myid, limPL_excw(ugrid,1,myid), limPL_excw(ugrid,2,myid)
close(unit=60)
end if
if (myid .eq. 0) then
open(unit=61,file='y_limits_phys_0.dat')
!rewind 60
write(61,*) 'myid, nyuIB1, nyuIB2 =', myid,             nyuIB1(myid),             nyuIB2(myid)
write(61,*) 'myid,  jlim1,  jlim2 =', myid,      jlim(1,ugrid,sband),      jlim(2,ugrid,sband)
write(61,*) 'myid,  jgal1,  jgal2 =', myid,            jgal(ugrid,1),            jgal(ugrid,2)
write(61,*) 'myid,limPL1i,limPL2i =', myid, limPL_incw(ugrid,1,myid), limPL_incw(ugrid,2,myid)
write(61,*) 'myid,limPL1e,limPL2e =', myid, limPL_excw(ugrid,1,myid), limPL_excw(ugrid,2,myid)
close(unit=61)
end if
call MPI_BARRIER(MPI_COMM_WORLD,ierr)
...
!  immersed boundaries, and then the list is empty.
  ...
  integer j,iproc,ilist,ilist2, jj
  integer nlist_su, nlist_sv, ny_columnv, ny_columnu
  integer :: nlist_fu, nlist_fv
  integer :: s_nlist, f_nlist
  integer :: topbottom
  logical :: firstlimit
  integer, allocatable:: inputNu(:), inputNv(:)                 ! Message passing array containing input parameters
  integer, allocatable:: list_su1(:,:),list_su2(:,:)
  integer, allocatable:: list_sv1(:,:),list_sv2(:,:)
  real(8), allocatable:: A_ib1(:,:,:),A_ib2(:,:,:)
  integer, allocatable:: list_su(:), list_sv(:)
  integer, allocatable:: list_fu(:), list_fv(:)
  integer, allocatable :: list_fu1(:,:), list_fu2(:,:)
  integer, allocatable :: list_fv1(:,:), list_fv2(:,:)
...
! MASTER
    ...
    if (myid==0) then
      ...
      !  NOT USED A_ib1 and A_ib2 stores the weights of the points in the imm boundaries.
      write(*,*) 'Creating geometry'
      call boundary_fibers
      write(*,*) 'Geomotry finished, back to start.f90'
  
      open(10,file=boundfname,form='unformatted',access='stream')
      read(10) Lx,Ly,Lz
      read(10) Ngal, nlist_su, nlist_sv, nlist_fu, nlist_fv, ny_columnv, ny_columnu, &
      & nyu11, nyu21, nyu12, nyu22, nyv11, nyv21, nyv12, nyv22
      read(10) nelements_y, dstz
      
      allocate(list_su1(3,nlist_su), list_su2(3,nlist_su))
      allocate(list_sv1(3,nlist_sv), list_sv2(3,nlist_sv))
      read(10) list_su1, list_su2  
      read(10) list_sv1, list_sv2

      allocate(list_fu1(9,nlist_fu), list_fu2(9,nlist_fu))
      allocate(list_fv1(9,nlist_fv), list_fv2(9,nlist_fv))
      read(10) list_fu1, list_fu2
      read(10) list_fv1, list_fv2

      allocate(list_wu1(3,nlist_fu), list_wu2(3,nlist_fu))
      allocate(list_wv1(3,nlist_fv), list_wv2(3,nlist_fv))
      read(10) list_wu1, list_wu2
      read(10) list_wv1, list_wv2
      ...
      ! 27.2.2020:
      ! Creating overlap regions for neighbouring procs 
      ! Thickness: jj, depending on the interpolation stencil in IBM
      !jj = 1
      !TODO change these statements using pandPL or something similar

      !   iproc = 0
      !    nyuIB1(iproc) =     nyu11                         !
      !    nyuIB2(iproc) =            planelim(ugrid,2,iproc)  + jj
      !    nyvIB1(iproc) =     nyv11 
      !    nyvIB2(iproc) =            planelim(vgrid,2,iproc)  + jj

      do iproc = 0, np-1
        if (iproc<np/2) then                                ! Lower riblets
          nyuIB1(iproc) = max(nyu11, planelim(ugrid,1,iproc)) !- jj
          nyuIB2(iproc) = min(nyu21, planelim(ugrid,2,iproc)) !+ jj
          nyvIB1(iproc) = max(nyv11, planelim(vgrid,1,iproc)) !- jj
          nyvIB2(iproc) = min(nyv21, planelim(vgrid,2,iproc)) !+ jj
        else                                                ! Upper riblets
          nyuIB1(iproc) = max(nyu12, planelim(ugrid,1,iproc)) !- jj
          nyuIB2(iproc) = min(nyu22, planelim(ugrid,2,iproc)) !+ jj
          nyvIB1(iproc) = max(nyv12, planelim(vgrid,1,iproc)) !- jj
          nyvIB2(iproc) = min(nyv22, planelim(vgrid,2,iproc)) !+ jj
        end if
      end do
         
      !   iproc = np-1
      !    nyuIB1(iproc) =            planelim(ugrid,1,iproc)  - jj
      !    nyuIB2(iproc) =     nyu22
      !    nyvIB1(iproc) =            planelim(vgrid,1,iproc)  - jj
      !    nyvIB2(iproc) =     nyv22
      ...
      do iproc = 1, np/2-1
                ...
                nlist_s = 0
        allocate(list_su(nlist_su), list_sv(nlist_sv))
        ...
        !ugrid-----------------------
        do ilist = 1, nlist_su
          j = list_su1(3,ilist)                          ! 1:i 2:k 3:j
          if (j>=nyuIB1(iproc) .and. j<=nyuIB2(iproc)) then
            nlist_s(ugrid) = nlist_s(ugrid) + 1
            list_su(nlist_s(ugrid)) = ilist
          end if
        end do
        !vgrid-----------------------
        do ilist = 1, nlist_sv
          j = list_sv1(3,ilist)                          ! 1:i 2:k 3:j
          if (j>=nyvIB1(iproc) .and. j<=nyvIB2(iproc)) then
            nlist_s(vgrid) = nlist_s(vgrid) + 1
            list_sv(nlist_s(vgrid)) = ilist
          end if
        end do

                nlist_f = 0
        allocate(list_fu(nlist_fu), list_fv(nlist_fv))
        ! Checks the points of the ghost points that belong to planes of the proc 'iproc'
        ! If is, added to a list
        !ugrid-----------------------
        do ilist = 1, nlist_fu
          j = list_fu1(3,ilist)                           ! 1:i 2:k 3:j 4,5,6: i,j,k of p1
          if (j>=nyuIB1(iproc) .and. j<=nyuIB2(iproc)) then
            nlist_f(ugrid) = nlist_f(ugrid) + 1
            list_fu(nlist_f(ugrid)) = ilist
          end if
        end do
        !vgrid-----------------------
        do ilist = 1, nlist_fv
          j = list_fv1(3,ilist)
          if (j>=nyvIB1(iproc) .and. j<=nyvIB2(iproc)) then
            nlist_f(vgrid) = nlist_f(vgrid) + 1
            list_fv(nlist_f(vgrid)) = ilist
          end if 
        end do
        
 
        ! The list is reordered and stored in a local list
        allocate(s_list_u(3,nlist_s(ugrid)))
        allocate(s_list_v(3,nlist_s(vgrid)))

        allocate(f_list_u(9,nlist_f(ugrid)))
        allocate(f_list_v(9,nlist_f(vgrid)))

        allocate(w_list_u(3,nlist_f(ugrid)))
        allocate(w_list_v(3,nlist_f(vgrid)))

        do ilist2 = 1, nlist_s(ugrid)
          ilist = list_su(ilist2)
          s_list_u(1,ilist2) = list_su1(1,ilist) ! i of solid point
          s_list_u(2,ilist2) = list_su1(2,ilist) ! k of solid point
          s_list_u(3,ilist2) = list_su1(3,ilist) ! j of solid point
        end do

        do ilist2 = 1, nlist_f(ugrid)
          ilist = list_fu(ilist2)
          f_list_u(1,ilist2) = list_fu1(1,ilist) ! i of forcing point
          f_list_u(2,ilist2) = list_fu1(2,ilist) ! k
          f_list_u(3,ilist2) = list_fu1(3,ilist) ! j
          f_list_u(4,ilist2) = list_fu1(4,ilist) ! i of fluid point 2
          f_list_u(5,ilist2) = list_fu1(5,ilist) ! k 
          f_list_u(6,ilist2) = list_fu1(6,ilist) ! j
          f_list_u(7,ilist2) = list_fu1(7,ilist) ! i of fluid point 3
          f_list_u(8,ilist2) = list_fu1(8,ilist) ! k 
          f_list_u(9,ilist2) = list_fu1(9,ilist) ! j

          w_list_u(1,ilist2) = list_wu1(1,ilist) ! weighting of boundary point 1
          w_list_u(2,ilist2) = list_wu1(2,ilist) ! weighting of    fluid point 2
          w_list_u(3,ilist2) = list_wu1(3,ilist) ! weighting of    fluid point 3
        end do

        do ilist2 = 1,nlist_s(vgrid)
          ilist = list_sv(ilist2)
          s_list_v(1,ilist2) = list_sv1(1,ilist) ! i
          s_list_v(2,ilist2) = list_sv1(2,ilist) ! k
          s_list_v(3,ilist2) = list_sv1(3,ilist) ! j
        end do

        do ilist2 = 1, nlist_f(vgrid)
          ilist = list_fv(ilist2)
          f_list_v(1,ilist2) = list_fv1(1,ilist) ! i of forcing point
          f_list_v(2,ilist2) = list_fv1(2,ilist) ! k 
          f_list_v(3,ilist2) = list_fv1(3,ilist) ! j
          f_list_v(4,ilist2) = list_fv1(4,ilist) ! i of fluid point 2
          f_list_v(5,ilist2) = list_fv1(5,ilist) ! k 
          f_list_v(6,ilist2) = list_fv1(6,ilist) ! j
          f_list_v(7,ilist2) = list_fv1(7,ilist) ! i of fluid point 3
          f_list_v(8,ilist2) = list_fv1(8,ilist) ! k 
          f_list_v(9,ilist2) = list_fv1(9,ilist) ! j

          w_list_v(1,ilist2) = list_wv1(1,ilist) ! weighting of boundary point 1
          w_list_v(2,ilist2) = list_wv1(2,ilist) ! weighting of    fluid point 2
          w_list_v(3,ilist2) = list_wv1(3,ilist) ! weighting of    fluid point 3
        end do
        
        deallocate(list_su, list_sv)
        deallocate(list_fu, list_fv)
        
        !--------------------------------------------
        ! Send info to the corresponding pocs
        !--------------------------------------------
        ! The list with 'imm boundary points you have to handle with' is sent to every proc
        inputNu(1) = nlist_s(ugrid)
        inputNv(1) = nlist_s(vgrid)
        ...
        ! The list with 'ghost points you have to handle with' is sent to every proc
        inputNu(2*np+1+5) = nlist_f(ugrid)
        inputNv(2*np+1+5) = nlist_f(vgrid)
        ...
        ! ATTENTION nlist_s ugrid and vgrid are equal, otherwise it wont work
        call MPI_SEND(s_list_u,3*nlist_s(ugrid),MPI_INTEGER,iproc,22000+iproc,MPI_COMM_WORLD,ierr)
        call MPI_SEND(s_list_v,3*nlist_s(vgrid),MPI_INTEGER,iproc,23000+iproc,MPI_COMM_WORLD,ierr)
        call MPI_SEND(f_list_u,9*nlist_f(ugrid),MPI_INTEGER,iproc,24000+iproc,MPI_COMM_WORLD,ierr)
        call MPI_SEND(f_list_v,9*nlist_f(vgrid),MPI_INTEGER,iproc,25000+iproc,MPI_COMM_WORLD,ierr)
        call MPI_SEND(w_list_u,3*nlist_f(ugrid),MPI_REAL8  ,iproc,26000+iproc,MPI_COMM_WORLD,ierr)
        call MPI_SEND(w_list_v,3*nlist_f(vgrid),MPI_REAL8  ,iproc,27000+iproc,MPI_COMM_WORLD,ierr)

        deallocate(s_list_u,s_list_v)
        deallocate(f_list_u,f_list_v)
        deallocate(w_list_u,w_list_v)
      end do
      ...
      do iproc = np/2, np-1
        ...
        do ilist2 = 1,nlist_s(ugrid)
          ilist = list_su(ilist2)
          s_list_u(1,ilist2) = list_su2(1,ilist)  ! i
          s_list_u(2,ilist2) = list_su2(2,ilist)  ! k
          s_list_u(3,ilist2) = list_su2(3,ilist)  ! j
        end do

        do ilist2 = 1, nlist_f(ugrid)
          ilist = list_fu(ilist2)
          f_list_u(1,ilist2) = list_fu2(1,ilist) ! i of forcing point
          f_list_u(2,ilist2) = list_fu2(2,ilist) ! k
          f_list_u(3,ilist2) = list_fu2(3,ilist) ! j
          f_list_u(4,ilist2) = list_fu2(4,ilist) ! i of fluid point 2
          f_list_u(5,ilist2) = list_fu2(5,ilist) ! k 
          f_list_u(6,ilist2) = list_fu2(6,ilist) ! j 
          f_list_u(7,ilist2) = list_fu2(7,ilist) ! i of fluid point 3
          f_list_u(8,ilist2) = list_fu2(8,ilist) ! k 
          f_list_u(9,ilist2) = list_fu2(9,ilist) ! j 

          w_list_u(1,ilist2) = list_wu2(1,ilist) ! weighting of boundary point 1
          w_list_u(2,ilist2) = list_wu2(2,ilist) ! weighting of    fluid point 2
          w_list_u(3,ilist2) = list_wu2(3,ilist) ! weighting of    fluid point 3
        end do

        do ilist2 = 1,nlist_s(vgrid)
          ilist = list_sv(ilist2)
          s_list_v(1,ilist2) = list_sv2(1,ilist)  ! i
          s_list_v(2,ilist2) = list_sv2(2,ilist)  ! k
          s_list_v(3,ilist2) = list_sv2(3,ilist)  ! j
        end do

        do ilist2 = 1, nlist_f(vgrid)
          ilist = list_fv(ilist2)
          f_list_v(1,ilist2) = list_fv2(1,ilist) ! i of forcing point
          f_list_v(2,ilist2) = list_fv2(2,ilist) ! k
          f_list_v(3,ilist2) = list_fv2(3,ilist) ! j
          f_list_v(4,ilist2) = list_fv2(4,ilist) ! i of fluid point 2
          f_list_v(5,ilist2) = list_fv2(5,ilist) ! k 
          f_list_v(6,ilist2) = list_fv2(6,ilist) ! j
          f_list_v(7,ilist2) = list_fv2(7,ilist) ! i of fluid point 3
          f_list_v(8,ilist2) = list_fv2(8,ilist) ! k 
          f_list_v(9,ilist2) = list_fv2(9,ilist) ! j

          w_list_v(1,ilist2) = list_wv2(1,ilist) ! weighting of boundary point 1
          w_list_v(2,ilist2) = list_wv2(2,ilist) ! weighting of    fluid point 2
          w_list_v(3,ilist2) = list_wv2(3,ilist) ! weighting of    fluid point 3
        end do         
      ...
      do ilist2 = 1,nlist_s(ugrid)
        ilist = list_su(ilist2)
        s_list_u(1,ilist2) = list_su1(1,ilist)  ! i
        s_list_u(2,ilist2) = list_su1(2,ilist)  ! k
        s_list_u(3,ilist2) = list_su1(3,ilist)  ! j
      end do
      ...
      do ilist2 = 1, nlist_f(ugrid)
        ilist = list_fu(ilist2)
        f_list_u(1,ilist2) = list_fu1(1,ilist) ! i
        f_list_u(2,ilist2) = list_fu1(2,ilist) ! k
        f_list_u(3,ilist2) = list_fu1(3,ilist) ! j
        f_list_u(4,ilist2) = list_fu1(4,ilist) ! i 2
        f_list_u(5,ilist2) = list_fu1(5,ilist) ! k
        f_list_u(6,ilist2) = list_fu1(6,ilist) ! j
        f_list_u(7,ilist2) = list_fu1(7,ilist) ! i 3
        f_list_u(8,ilist2) = list_fu1(8,ilist) ! k
        f_list_u(9,ilist2) = list_fu1(9,ilist) ! j

        w_list_u(1,ilist2) = list_wu1(1,ilist)
        w_list_u(2,ilist2) = list_wu1(2,ilist)
        w_list_u(3,ilist2) = list_wu1(3,ilist)
      end do

      do ilist2 = 1, nlist_f(vgrid)
        ilist = list_fv(ilist2)
        f_list_v(1,ilist2) = list_fv1(1,ilist) ! i
        f_list_v(2,ilist2) = list_fv1(2,ilist) ! k
        f_list_v(3,ilist2) = list_fv1(3,ilist) ! j
        f_list_v(4,ilist2) = list_fv1(4,ilist) ! i 2
        f_list_v(5,ilist2) = list_fv1(5,ilist) ! k
        f_list_v(6,ilist2) = list_fv1(6,ilist) ! j
        f_list_v(7,ilist2) = list_fv1(7,ilist) ! i 3
        f_list_v(8,ilist2) = list_fv1(8,ilist) ! k
        f_list_v(9,ilist2) = list_fv1(9,ilist) ! j

        w_list_v(1,ilist2) = list_wv1(1,ilist)
        w_list_v(2,ilist2) = list_wv1(2,ilist)
        w_list_v(3,ilist2) = list_wv1(3,ilist)
      end do 

      deallocate(list_su, list_sv)
      deallocate(list_su1,list_su2, list_sv1,list_sv2)
      deallocate(list_fu, list_fv)     
      deallocate(list_fu1, list_fu2, list_fv1, list_fv2)
      deallocate(list_wu1, list_wu2, list_wv1, list_wv2)

! SLAVES

    else
      ! The information is received by every proc
      !write(*,*) 'The information is received by every proc'
      ...
      nlist_s(ugrid) = inputNu(1)
      nlist_s(vgrid) = inputNv(1)
      do iproc = 0,np-1
      ...
      nlist_f(ugrid) = inputNu(2*np+1+5)
      nlist_f(vgrid) = inputNv(2*np+1+5)
      ...
      allocate(w_list_v(3,nlist_f(vgrid)))
      ! ATTENTION nlist_s ugrid and vgrid are equal, otherwise it wont work
      call MPI_RECV  (s_list_u,3*nlist_s(ugrid),MPI_INTEGER,0,22000+myid,MPI_COMM_WORLD,status,ierr)
      call MPI_RECV  (s_list_v,3*nlist_s(vgrid),MPI_INTEGER,0,23000+myid,MPI_COMM_WORLD,status,ierr)
      call MPI_RECV  (f_list_u,9*nlist_f(ugrid),MPI_INTEGER,0,24000+myid,MPI_COMM_WORLD,status,ierr)
      call MPI_RECV  (f_list_v,9*nlist_f(vgrid),MPI_INTEGER,0,25000+myid,MPI_COMM_WORLD,status,ierr)
      call MPI_RECV  (w_list_u,3*nlist_f(ugrid),MPI_REAL8  ,0,26000+myid,MPI_COMM_WORLD,status,ierr)
      call MPI_RECV  (w_list_v,3*nlist_f(vgrid),MPI_REAL8  ,0,27000+myid,MPI_COMM_WORLD,status,ierr)
    end if
    deallocate(inputNu)
    deallocate(inputNv)
...
subroutine proc_lims_planes(myid)
  ...
  ! 18.5.2020
  ! Due to the interpolation stencil of IBM, the load at bottom and top band are tripled
  !load_band(1) = 3*(Ngal(1,1)+2)*Ngal(2,1)*(Ngal(4,1)-N(4,1-1)) ! Number of points 3*(nx+2)*nz*ny(band)
  !load_band(2) =   (Ngal(1,2)+2)*Ngal(2,2)*(Ngal(4,2)-N(4,2-1)) ! Number of points   (nx+2)*nz*ny(band)
  !load_band(3) = 3*(Ngal(1,3)+2)*Ngal(2,3)*(Ngal(4,3)-N(4,3-1)) ! Number of points 3*(nx+2)*nz*ny(band)
