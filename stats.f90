...
!   real(8) x   (N(1,bandPL(myid))+2,N(2,bandPL(myid)),limPL_incw(grid,1,myid)           :limPL_incw(grid,2,myid))
  real(8) YmC (1 ,dnz ,limPL_incw(grid,1,myid)-Ny(grid,0):limPL_incw(grid,2,myid)-Ny(grid,0))
  real(8) Y2mC(1 ,dnz ,limPL_incw(grid,1,myid)-Ny(grid,0):limPL_incw(grid,2,myid)-Ny(grid,0))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  real(8) x   (igal,kgal, limPL_incw(grid,1,myid)                 : limPL_incw(grid,2,myid))
  !real(8) x   (N(1,bandPL(myid))+2,N(2,bandPL(myid)),limPL_incw(grid,1,myid)           :limPL_incw(grid,2,myid))
  real(8) YmC (1 ,dnz ,-limPL_incw(grid,2,myid)+Ny(grid,nband)+1:-limPL_incw(grid,1,myid)+Ny(grid,nband)+1)
  real(8) Y2mC(1 ,dnz ,-limPL_incw(grid,2,myid)+Ny(grid,nband)+1:-limPL_incw(grid,1,myid)+Ny(grid,nband)+1)
  real(8) xsign
    ...
    do k = 1,dnz
      do i = 1,1
        do iz = 1,ntilez
          ...
          do ix = 1,ntilex
            !i1 = indior(i+1*(ix-1))
            !i1 = indior(1*ix+2-i)                 ! Opposite direction to bottom wall
            i1 = i+1*(ix-1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if (myid/=0) then   ! SLAVES
    ...
    ! Conditional statistics
      msizeu = 1*dnz*(limPL_incw(ugrid,2,myid)-limPL_incw(ugrid,1,myid)+1)
      msizev = 1*dnz*(limPL_incw(vgrid,2,myid)-limPL_incw(vgrid,1,myid)+1)
      msizep = 1*dnz*(limPL_incw(pgrid,2,myid)-limPL_incw(pgrid,1,myid)+1)
      if(msizeu>0)then
    ...
    msizeu = 1*dnz*(limPL_incw(ugrid,2,myid)-limPL_incw(ugrid,1,myid)+1)
    if(msizeu>0)then
      allocate(xCu  (1,dnz,0:Ny(ugrid,1)-Ny(ugrid,0)),buffCu(1,dnz,0:Ny(ugrid,1)-Ny(ugrid,0)))
      allocate(xCv  (1,dnz,0:Ny(vgrid,1)-Ny(vgrid,0)),buffCv(1,dnz,0:Ny(vgrid,1)-Ny(vgrid,0)))
      allocate(xCp  (1,dnz,0:Ny(pgrid,1)-Ny(pgrid,0)),buffCp(1,dnz,0:Ny(pgrid,1)-Ny(pgrid,0)))
      call recvXC(xCu ,UmC  ,buffCu ,1,dnz,ugrid,status,ierr)
      call recvXC(xCu ,U2mC ,buffCu ,1,dnz,ugrid,status,ierr)
      call recvXC(xCv ,VmC  ,buffCv ,1,dnz,vgrid,status,ierr)
      call recvXC(xCv ,V2mC ,buffCv ,1,dnz,vgrid,status,ierr)
      call recvXC(xCu ,WmC  ,buffCu ,1,dnz,ugrid,status,ierr)
      call recvXC(xCu ,W2mC ,buffCu ,1,dnz,ugrid,status,ierr)
      call recvXC(xCp ,PmC  ,buffCp ,1,dnz,pgrid,status,ierr)
      call recvXC(xCp ,P2mC ,buffCp ,1,dnz,pgrid,status,ierr)
      call recvXC(xCu ,UVmC ,buffCu ,1,dnz,ugrid,status,ierr)
      call recvXC(xCu ,UWmC ,buffCu ,1,dnz,ugrid,status,ierr)
      call recvXC(xCv ,VWmC ,buffCv ,1,dnz,vgrid,status,ierr)
      call recvXC(xCv ,wxmC ,buffCv ,1,dnz,vgrid,status,ierr)
      call recvXC(xCv ,wx2mC,buffCv ,1,dnz,vgrid,status,ierr)
