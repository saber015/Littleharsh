...
subroutine LUsolU(x,Lux,grid,myid)
   !----------------------------------------------------------------------*
   ...
   do iband = sband,eband
      do column = 1,columns_num(iband,myid)
         ...
         !        Lux(iband)%f(j,column)=Lux(iband)%f(j,column)/axz(column,iband)
   ...
   call immersed_boundaries_U(x,Lux,grid,myid)

  
   ! -Original
   do iband = sband,eband
      do column = 1,columns_num(iband,myid)
         x(iband)%f(jlim(1,grid,iband),column)=x(iband)%f(jlim(1,grid,iband),column)*ay(2,jlim(1,grid,iband),iband)
         do j = jlim(1,grid,iband)+1,jlim(2,grid,iband)
            x(iband)%f(j,column) = (x(iband)%f(j,column)-ay(1,j,iband)*x(iband)%f(j-1,column))*ay(2,j,iband)
         end do
         do j = jlim(2,grid,iband)-1,jlim(1,grid,iband),-1
            x(iband)%f(j,column) =  x(iband)%f(j,column)-ay(3,j,iband)*x(iband)%f(j+1,column)
         end do
      end do
   enddo
...
subroutine LUsolV(x,Lux,grid,myid)
   !----------------------------------------------------------------------*
   !      GIVEN a_j=L*U AND a_j*x_j=f_j FOR k=nstart:nend
   !      FIRST SOLVE L_j*y_j=f_j AND THEN U_j*x_j=y_j
   ! INPUT m_j number of unknowns
   ...
   call immersed_boundaries_V(x,Lux,grid,myid)
...
subroutine immersed_boundaries_U(rhsu,Lu,grid,myid)
   ...
   do ilist = 1,nlist_ib(grid)
      i = list_ib(1,ilist,grid)
      k = list_ib(2,ilist,grid)
      j = list_ib(3,ilist,grid)

      rhsuIB(i,k,j) = 0d0

      if (j==0.OR.j==Ngal(4,3)-dsty+1) then
         rhsuIB(i,k,j) = -beta*LuIB(i,k,j)   !-beta/dyub2*rhsuIB(i,k,j+1)
      endif
   enddo
   ...
   !        rhsuIB(i,k,j) = 0d0
   !
   !        if (j==Ngal(4,3)-dsty+1) then
   !          rhsuIB(i,k,j) = -beta*LuIB(i,k,j)  !-beta/dyut2*rhsuIB(i,k,j+1)   !-LuIB(i,k,j)
   !        endif
   !      enddo
   !
   !
...
subroutine immersed_boundaries_V(rhsu,Lu,grid,myid)
   ...
   do ilist = 1,nlist_ib(grid)
      ...
      if (j==-1.OR.j==Ngal(3,3)-dsty+2) then
         rhsuIB(i,k,j) = -beta*LuIB(i,k,j)   !-beta/dyvb2*rhsuIB(i,k,j+1) !-LuIB(i,k,j)
      endif
   enddo
   ...
   !        rhsuIB(i,k,j) = 0d0
   !
   !        if (j==Ngal(3,3)-dsty+2) then
   !          rhsuIB(i,k,j) = -beta*LuIB(i,k,j)      !-beta/dyvt2*rhsuIB(i,k,j+1)  !-LuIB(i,k,j)
   !        endif
   !      enddo
   !
   !
...
subroutine LU_build(nystart,nyend,grid,myid,iband,axz,ay)
   ...
   if (grid==ugrid) then
      ...
      ay(:,:,iband)=ay(:,:,iband)
   !        ay(3,nystart,iband) = -1d0
   !        ay(1,nyend  ,iband) = -1d0
